...
def get_indexed_window_average(arr: np.ndarray, idxs: np.ndarray, window_size: int):
...
def compute_metrics(
    ...
    # max_k_window
    k_window_value = sn.uniform_filter1d(
        key_data, size=max_k_window, axis=0, mode="nearest"
    )
...
def compute_metrics_agg(log_dfs: List[pd.DataFrame], key: str, end_step: int, **kwargs):
...
@click.command()
@click.option(
    "--input", "-i", required=True, help="Root logging dir, contains train_* dirs"
)
    ...
    with JsonLogger(metric_log_path) as json_logger:
        ...
        while True:
            ...
            if last_log is not None:
                last_log_idx = log_dfs[0].index[
                    log_dfs[0][index_key] <= last_log[index_key]
                ][-1]
